...
# pcap.pyx
#
# $Id: pcap.pyx 80 2006-08-13 05:39:56Z dugsong $
...
cdef extern from "pcap.h":
    ...
    void	pcap_dump(unsigned char *, pcap_pkthdr *, unsigned char *)
    unsigned char*   pcap_dump_file(pcap_dumper_t *)
...
cdef void __pcap_handler(void *arg, pcap_pkthdr *hdr, char *pkt):
    ...
    try:
        if ctx.pcap_dumper != <pcap_dumper_t*>0:
            pcap_dump(<unsigned char*> ctx.pcap_dumper, hdr, <unsigned char*> pkt)
...
cdef class pcap:
    ...
    cdef int __dloff
    cdef pcap_dumper_t *__pcap_dumper
    ...
    def __init__(self, name=None, snaplen=65535, promisc=True,
        ...
        self.__pcap_dumper = <pcap_dumper_t*> 0
        ...
        if not self.__pcap:
            raise str(self.__ebuf) + str(pcap_ex_name(p))
    ...
    def dumpopen(self, fname):
        self.__fileName = strdup(fname)
        self.__pcap_dumper = pcap_dump_open(self.__pcap, self.__fileName)
    
    def dumpclose(self):
        if self.__pcap_dumper != <pcap_dumper_t*>0:
            pcap_dump_close(self.__pcap_dumper)
    ...
    def dispatch(self, cnt, callback, *args):
        ...
        *args    -- optional arguments passed to callback on execution
        ...
        ctx.args = <void *>args
        ctx.got_exc = 0
        ctx.pcap_dumper = self.__pcap_dumper
    ...
    def __next__(self):
        ...
        while 1:
            ...
            if self.__pcap_dumper != <pcap_dumper_t*>0:
                pcap_dump(<unsigned char*> self.__pcap_dumper, hdr, <unsigned char*> pkt)
                pcap_dump_flush(self.__pcap_dumper)
